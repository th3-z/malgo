...
CREATE TABLE user_series( -- User series join table
    user_series_id INTEGER NOT NULL PRIMARY KEY,
...
CREATE TABLE series_type( -- TV, Movie, etc.
    series_type_id INTEGER NOT NULL PRIMARY KEY,
    name VARCHAR(255) UNIQUE,
);

CREATE TABLE series_status( -- Airing, Aired, etc
    series_status_id INTEGER NOT NULL PRIMARY KEY,
    name VARCHAR(255) UNIQUE,
);
...
CREATE TABLE storage_type( -- HDD, DVD, etc.
    storage_type_id INTEGER NOT NULL PRIMARY KEY,
    name VARCHAR(255) UNIQUE,
);
...
CREATE TABLE user_series_tag ( -- User series tags join table
	user_series_tag_id INTEGER NOT NULL PRIMARY KEY,
